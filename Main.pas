...
begin
 ...
 if NugnaZamena and (min<>100000000000) and (pluss>0) then
  begin
   ...
   Ap:=CreateOleObject('Excel.Application');
   Ap.Workbooks.Open(way,0,false); //Открываем опись ведомостей замены
   MainForm.Label2.Caption:='Процедура замены ВИ...открываем опись ведомостей замены...';
...
//   If pEx=2 then NVed:=1 else inc(NVed);
   ...
   //ShowMessage('Путь к файлу ОБРАЗЦА ведомости замены: '+wayEX);
   way:=MainForm.Edit15.Text+NowDate+'\VZAMEN_'+IntToStr(Nved)+'.xlsx';
   //ShowMessage('Путь к файлу ведомости замены: '+way);
   Ap:=CreateOleObject('Excel.Application');
   ...
   //ShowMessage('Путь к файлу ведомости замены ПЕРЕД СОХРАНЕНИЕМ: '+way);
...
begin
 ...
 MainForm.Label2.Caption:='Перевод сотрудника...оформление аттестата...открываем карточку сотрудника';
 ShowMessage('Переменная OldName: '+OldName);
 if FileExists(OLdName) then
  begin
   ...
   MainForm.Label2.Caption:='Перевод сотрудника...оформление аттестата...регистрация в описи аттестатов';
   ...
   MainForm.Label2.Caption:='Перевод сотрудника...оформление аттестата...создание аттестата сотрудника';
